...
float validation_alpha(vector<int> &feature){
    ...
    cout<<"Correctness "<<(correct/table.size())<<endl;

    return correct/table.size();
}
...
void forward(){
    ...
    while(candidate.size()>0){
        ...
        // the best result is store in accuracy_level and accuracy_candicate
        for(int s=0;s<candidate.size();s++){
            cout<<"push "<<candidate[s]<<endl;
            tmp.push_back(candidate[s]);
            float tmp_acc = validation(tmp);
        ...
        // after we have a sure decision, add accuracy_candicate into feature_table[i]
...
int main(){
    ...
    float input;
    FILE *fp = fopen("CS205_SMALLtestdata__68.txt","r");
    vector<float> v;
    vector<float> min;
    vector<float> max;
    vector<float> diff;
